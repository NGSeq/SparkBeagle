...
public final class PlinkGenMap implements GeneticMap {
     ...
     */
    public static PlinkGenMap fromPlinkMapFile(FSDataInputStream mapFile) {
     ...
     */
    public static PlinkGenMap fromPlinkMapFile(FSDataInputStream mapFile, String chrom) {
        chrom = chrom.trim();
   ...
   private static <E> Filter<E> singletonFilter(final E singleton) {
    ...
    private static List<List<String>> divideByChrom(FSDataInputStream mapFile,
            Filter<String> chromFilter) {
        int initialMapSize = 200;
        List<List<String>> chromList = new ArrayList<>(25);
        try (FileIt<String> it = InputIt.fromGzipFile(mapFile, false)) {
            while (it.hasNext()) {
                String line = it.next();
     ...
     */
    public int nMapPositions(int chr) {
        //checkChromIndex(chrom);
        int chrom = 0;
        return basePos[chrom].length;
    }

    public int getOverlapLength(int chr, int startbase, int endpbase) {
        //checkChromIndex(chrom)
        int chrom = 0;
        int start = closestIndex(chrom,startbase);
        int end =  closestIndex(chrom,endpbase);
        return Arrays.copyOfRange(basePos[chrom],start, end).length;
    }
    /**
     ...
     */
    public int index2BasePos(int chr, int index) {
        //checkChromIndex(chrom);
        int chrom = 0;
     ...
     * {@code index < 0 || index >= this.nMapPositions(chrom)}
     */
    public double index2GenPos(int chr, int index) {
        //checkChromIndex(chrom);
        int chrom = 0;
     ...
     * {@code chrom < 0 || chrom >= ChromIds.instance().size()}
     */
    public int closestIndex(int chr, int basePosition) {
        //checkChromIndex(chrom);
        int chrom = 0;
    ...
    @Override
    public double genPos(int chr, int basePosition) {
        //checkChromIndex(chrom);
        int chrom = 0;
    ...
    @Override
    public int basePos(int chr, double geneticPosition) {
        //checkChromIndex(chrom);
        int chrom = 0;
        assert basePos[chrom].length>=2;
        assert basePos[chrom].length==genPos[chrom].length;
